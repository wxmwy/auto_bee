...
IMAGES, SOUNDS, HITMASKS = spider_utils.load()
...
WEB_WIDTH = IMAGES['pipe'][2].get_width()
WEB_HEIGHT = IMAGES['pipe'][2].get_height()
...
def checkCrash(player, upperPipes, lowerPipes, typePipes):
    ...
    if player['y'] + player['h'] >= BASEY - 1:
        ...
        for uPipe, lPipe, typep in zip(upperPipes, lowerPipes, typePipes):
            ...
            uwebRect = pygame.Rect(uPipe['x'], 0, WEB_WIDTH, WEB_HEIGHT)
            # player and upper/lower pipe hitmasks
            pHitMask = HITMASKS['player'][pi]
            uHitmask = HITMASKS['pipe'][typep*2+0]
            uweb = HITMASKS['pipe'][typep*2+2]
            lHitmask = HITMASKS['pipe'][typep*2+1]

            # if bird collided with upipe or lpipe
            uCollide = pixelCollision(playerRect, uPipeRect, pHitMask, uHitmask)
            lCollide = pixelCollision(playerRect, lPipeRect, pHitMask, lHitmask)
            uwebc = pixelCollision(playerRect, uwebRect, pHitMask, uweb)
            if uCollide or lCollide or uwebc:
                return True
